...
- Case-level метрики recall, miss rate, precision, specificity и целевой
  false-positive rate для зафиксированного валидатора SmartBugs.
- Воспроизводимый reentrancy-пример с локальной исправленной фикстурой,
  Markdown-экспортом, командами полной сессии и проверкой до и после исправления.
- Открытый путь обратной связи: свободные Issues, шаблон отзыва, усиленная
  проверка Pull Request и прямая приватная отправка отчётов об уязвимостях.

### Изменено

- В оба README добавлены короткие разделы с проверенной доказательной базой.
- Проверка административных функций сужена: обычные withdrawal, burn и
  preference-функции больше не помечаются как административные пути.
- Исправлены ограничители калибровки: сравнение с baseline без регрессий больше
  не создаёт ложное предупреждение о regression-like изменениях.
