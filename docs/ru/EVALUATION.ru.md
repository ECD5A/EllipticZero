...
запуска, baseline-сравнение, профили известных кейсов и предварительный просмотр
контекста провайдера.
...
| CI-проверка | Сохранённые запуски можно экспортировать в SARIF 2.1.0. | Code Scanning может показать ограниченные сигналы, включая привязку к строкам при наличии локальных подсказок, без автоматического превращения их в доказанные находки. |
...
валидатор проходит `6/6` целевых кейсов: пять размеченных положительных и один
синтетический отрицательный контроль. Он считает case-level recall, miss rate,
precision, specificity и целевой false-positive rate. Текущий результат:
`100%` recall, `0%` miss rate и `0%` целевой false-positive rate.

Метрики покрывают только выбранные кейсы access control, legacy reentrancy,
unchecked call, внешний вызов в цикле и bad randomness. Отрицательное основание
состоит из одного синтетического контроля, поэтому это не полный результат
SmartBugs, не доказательство эксплуатируемости и не общая оценка false-positive
rate. Полный путь с сессией, исправлением, baseline-сравнением и остаточными
сигналами описан в [практическом reentrancy-примере](CASE_STUDY_SMARTBUGS.ru.md).
...
Оценка с провайдером всё равно должна опираться на локальную доказательную
...
- ограничены ли ECC сигналы локальными вычислениями и явной неопределённостью
...
до запуска, продажи или развёртывания.
