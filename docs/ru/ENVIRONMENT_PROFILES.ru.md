...
- управляемый Solidity-компилятор в платформенном каталоге
  `.ellipticzero/tooling/solcx/<platform>`
...
В Linux и macOS используй shell-скрипт:

```bash
bash scripts/setup_local_lab.sh --profile lab
```

В WSL укажи `--venv-dir .venv-wsl`, чтобы Linux не использовал Windows-`.venv`.
Foundry, Echidna, Slither и Sage остаются опциональными системными инструментами;
`--doctor` показывает их фактическую готовность.
