...
investigation, reproducible evidence, and cautious reports across
smart-contract audit and ECC workflows.
...
- turn a vague smart-contract or ECC research idea into a bounded local session
...
- report snapshots compress repo-scale, remediation, and ECC deltas into first-screen review cues
...
4. Run one smart-contract static or repo-scale benchmark pack on a small local contract tree.
5. Run one ECC benchmark pack in `mock` mode.
...
- smart-contract audit and ECC research under one bounded orchestration model
