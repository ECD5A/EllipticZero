...
- выбран пакет `contract_static_benchmark_pack`
- parser и surface summary распознают синтетический ledger
- встроенная pattern-проверка не выдаёт issue family для контрольного контракта
- отчёт сохраняет границы ручной проверки и не заявляет доказанную безопасность

Хороший сигнал для оценки:

- regression scorecard отличает чистый контрольный пример от фикстур с ожидаемыми сигналами
